...
History:  -(04/02/2002) The lib was completed and released
            -(12/10/2002) Improved support for dateTime.iso8601 (howardlp@isis.vanderbilt.edu)
...
xmlrpc.prototype.setParameter = function(value,ptype) {
	...
	if(!ptype) {
		...
		if((kind = this.kindOf(value))== "array") {
            ...
            return {dtype: kind, values: dateToiso8601(value)}
		}else if((kind == "int") || (kind == "string") || (kind == "boolean") || (kind == "double") || (kind == "base64")){
		...
		if (ptype == "array") {
            ...
            return {dtype: ptype, values: dateToiso8601(value)}
		}else if((ptype == "int") || (ptype == "string") || (ptype == "boolean") || (ptype == "double") || (ptype == "base64")){
...
xmlrpc.prototype.parseValues = function(obj) {
	...
	if (obj.nodeName == "value") {
		...
		if (obj.nodeType == 3) {
			...
			name = obj.nodeName;
			if ((name == "string") or (name == "base64")) {
				//return {dtype: name,values: unescape(obj.childNodes[0].nodeValue.trim())};
				// lph - handle no value (e.g., <string></string> as "")
				return {dtype: name,values: (obj.childNodes.length > 0) ? obj.childNodes[0].nodeValue.trim() : ""};
			}else if (name == "boolean") {
				var r = obj.childNodes[0].nodeValue.trim();
				return {dtype: name,values: (r==1)?(true):(false)};
			}else if ((name == "int") or (name == "i4")) return {dtype: "int",values: parseInt(obj.childNodes[0].nodeValue.trim())};
			else if(name == "double") return {dtype: "double", values: parseFloat(obj.childNodes[0].nodeValue.trim())};
			else if(name == "dateTime.iso8601") return {dtype: "dateTime",values: iso8601Todate(obj.childNodes[0].nodeValue.trim())};
